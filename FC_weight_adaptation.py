...
class FC(nn.Module):
    def __init__(self, modality=[], ask_dim = 24,
        ...
        self.ask_w = WUnit(in_dim=ask_dim, mid_dim=3, out_dim=1)
        ...
        if 'face' in modality:
            ...
            self.face_w = WUnit(in_dim=3, mid_dim=3, out_dim=1)
        ...
        if 'top' in modality:
            ...
            self.top_w = WUnit(in_dim=3, mid_dim=3, out_dim=1)
        ...
        if 'pulse' in modality:
            ...
            self.pulse_w = WUnit(in_dim=2, mid_dim=3, out_dim=1)
        ...
        if 'bottom' in modality:
            ...
            self.bottom_w = WUnit(in_dim=bottom_dim, mid_dim=3, out_dim=1)
        if 'pulse' in modality:
    ...
    def forward(self, x1, x2=None, x3=None, x4=None, x5=None):
        ...
        if 'bottom' in self.modality:
            x4 = x4.view(b_size, -1)
            x4_w = self.bottom_w(x4)
            x4 = x4 * x4_w
            x = torch.cat([x, x4], dim=1)
        if 'pulse' in self.modality:
            x5 = x5.view(b_size, -1)
            x5 = self.pulse_fc(x5)
            x5_w = self.pulse_w(x5)
            x5 = x5 * x5_w
...
def main(modality=[], seed=2022, batch_size=32, n_epochs=300, lr=0.0003, early_stop=30):
